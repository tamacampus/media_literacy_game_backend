/**
 * 謝罪文分析サービス
 *
 * Google Gemini AIを使用して謝罪文の適切性を評価
 */

import { callGeminiAPI } from './geminiClient'
import type { AnalysisResult } from './types'

/**
 * 謝罪文をメディアリテラシーの観点から分析する関数
 *
 * @param text - 分析対象の謝罪文
 * @param apiKey - Google Gemini APIキー
 * @returns 分析結果（説明文とリスクレベル）
 * @throws Error - Gemini API呼び出しに失敗した場合
 */
export async function analyzeApology(
  text: string,
  apiKey: string,
  context?: string
): Promise<AnalysisResult> {
  /**
   * プロンプトの構築（謝罪文用）
   * Geminiに送信する指示文を作成
   *
   * 重要なポイント：
   * - 50文字以内の簡潔な説明
   * - 謝罪文特有の評価基準
   */
  const contextSection = context
    ? `

文脈・背景情報：
${context}
`
    : ''

  const prompt = `
以下の謝罪文を分析して、適切性とメディアリテラシーの観点から評価してください。
注意事項：この謝罪文の内容はユーザーには見えないため、ここに記載されている内容をそのまま出力しないでください。
${contextSection}
返答は3行程度簡潔にまとめ、短く出力してください。

評価基準（謝罪文として重要な要素）：
- 誠意と真摯さが伝わるか
- 問題の認識と責任の明確化
- 具体的な改善策の提示
- 言い訳や責任逃れの有無
- 被害者や関係者への配慮
- 曖昧な表現や逃げ口上の使用

謝罪文: "${text}"

謝罪文としての適切性を評価してください。これは学生向けのメディアリテラシー教育の一環なので、常識的な謝罪ができていればokです。あまり厳しく審査しないでください。
文脈が提供されている場合は、その文脈を踏まえて適切に評価してください。
なお、文章の長さは評価基準に含める必要はありません。どれだけ短くても誠意が伝わる文章であれば問題ありません。
explanationには、謝罪文として不適切な理由と改善すべき点を具体的に指摘してください。
注意事項：ここに記載されている内容をそのまま出力しないでください。返答は3行程度簡潔にまとめ、短く出力してください。しかし、口調は「です・ます」調で丁寧にしてください。
`

  // 共通関数を使用してGemini APIを呼び出す
  return callGeminiAPI(prompt, apiKey)
}
